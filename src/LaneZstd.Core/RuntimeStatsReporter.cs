...
public static class RuntimeStatsReporter
{
    ...
    public static string Format(string component, RuntimeStatsSnapshot snapshot, bool final = false)
    {
        return string.Create(
            ...
            $"{component} stats{(final ? " final" : string.Empty)} sessions active={snapshot.ActiveSessions} created={snapshot.SessionsCreated} closed={snapshot.SessionsClosed} timeout={snapshot.SessionsTimedOut} util={snapshot.SessionUtilization:F2} packets edge_in={snapshot.EdgePacketsIn} edge_out={snapshot.EdgePacketsOut} hub_in={snapshot.HubPacketsIn} hub_out={snapshot.HubPacketsOut} game_in={snapshot.GamePacketsIn} game_out={snapshot.GamePacketsOut} raw_out={snapshot.RawFramesOut} zstd_out={snapshot.CompressedFramesOut} raw_bytes_in={snapshot.RawBytesIn} framed_bytes_out={snapshot.FramedBytesOut} drop_oversize={snapshot.OversizeDrop} proto_err={snapshot.ProtocolError} zstd_err={snapshot.DecompressError} encode_ops={snapshot.EncodeOperations} encode_ms={snapshot.EncodeElapsedMilliseconds:F2} encode_us_avg={snapshot.EncodeAverageMicroseconds:F2} decode_ops={snapshot.DecodeOperations} decode_ms={snapshot.DecodeElapsedMilliseconds:F2} decode_us_avg={snapshot.DecodeAverageMicroseconds:F2} queue_in={snapshot.QueueEnqueued} queue_out={snapshot.QueueDequeued} queue_drop={snapshot.QueueDropped} queue_done={snapshot.QueueCompleted} unknown_session={snapshot.UnknownSession} sender_mismatch={snapshot.SessionSenderMismatch} pool_exhausted={snapshot.PortPoolExhausted} ratio={snapshot.CompressionRatio:F2} savings={snapshot.CompressionSavings:F2}");
    }
